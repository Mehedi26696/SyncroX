...
\begin{titlepage}
    ...
    % University Logo (placeholder - user should add logo)
    % \includegraphics[width=0.2\textwidth]{du_logo.png}\\[1cm]
    
    {\LARGE\textbf{University of Dhaka}}\\[0.4cm]
    {\large Department of Computer Science and Engineering}\\[1.5cm]
    
    \hrule height 2pt
    \vspace{0.3cm}
    {\Large\textbf{NETWORKING PROJECT PROPOSAL}}\\
    \vspace{0.3cm}
    \hrule height 2pt
    \vspace{1.5cm}
    
    {\Huge\textcolor{ducolor}{\textbf{SyncroX}}}\\[0.5cm]
    {\Large A Unified Real-Time Collaboration \& Communication Platform}\\[2cm]
    
    {\large\textbf{Course: CSE 3111 - Computer Networking Lab}}\\[2cm]
    
    \begin{minipage}{0.5\textwidth}
        \centering
        \textbf{Submitted By:}\\[0.3cm]
        \textcolor{red}{\textbf{H.M. Mehedi Hasan}}\\
        \textit{Roll: 13}\\[0.5cm]
        \textcolor{red}{\textbf{MD. Abu Bakar Siddique}}\\
        \textit{Roll: 47}
    \end{minipage}\\[2cm]
    ...
    \begin{minipage}{0.5\textwidth}
        ...
        \textcolor{red}{\textbf{Palash Roy}}\\
        \textit{Faculty, CSE Department}
    \end{minipage}\\[2cm]
    
    \vfill
    
    {\large\textbf{Submission Date: November 17, 2025}}
...
Code execution platforms require robust security measures. SyncroX demonstrates modern containerization techniques using Docker to provide:
\begin{itemize}
    \item Process isolation
    \item Resource limiting (CPU, memory)
    \item Network restriction
    \item Safe multi-language code execution
\end{itemize}

% ======================== PROBLEM STATEMENT ========================
\section{Problem Statement}

Modern collaborative development environments face several challenges:

\begin{enumerate}[label=\textbf{P\arabic*.}]
    \item \textbf{Fragmented Tools}: Developers typically use separate applications for chat (Slack), code collaboration (GitHub), file sharing (Dropbox), and code execution (online IDEs), leading to context switching and reduced productivity.
    
    \item \textbf{Network Performance}: File transfer systems often lack sophisticated congestion control, leading to inefficient bandwidth utilization and poor performance over varying network conditions.
    
    \item \textbf{Security Risks}: Online code execution platforms must carefully balance functionality with security to prevent malicious code execution.
    
    \item \textbf{Real-time Synchronization}: Maintaining consistency across multiple concurrent users editing shared resources requires careful protocol design and conflict resolution.
    
    \item \textbf{Educational Implementation}: Students learning networking concepts need practical, hands-on projects that implement theoretical algorithms in real-world scenarios.
\end{enumerate}

\textbf{SyncroX addresses these problems by:}
\begin{itemize}
    \item Providing a unified platform integrating all collaboration features
    \item Implementing production-grade congestion control algorithms with real-time metrics
    \item Using Docker containerization for secure, isolated code execution
    \item Designing custom TCP protocols optimized for different service types
    \item Serving as a comprehensive educational resource for networking students
\end{itemize}
...
\begin{itemize}
    \item Modern, intuitive user interface with dark theme
    \item Support for multiple programming languages (Python, C, C++, Java)
    \item Comprehensive error handling and logging
    \item Real-time dashboard for system monitoring
    \item Binary-safe file transfers with checksum validation
\end{itemize}
...
\begin{tikzpicture}[
    ...
    server/.style={rectangle, draw, fill=green!20, text width=2.5cm, text centered, rounded corners, minimum height=1cm},
...
% Servers
\node [server, below left=2cm and 1cm of frontend] (chat) {Chat Server\\Port 9009};
\node [server, below=2cm of frontend, xshift=-1cm] (collab) {Collab Server\\Port 9011};
\node [server, below=2cm of frontend, xshift=1cm] (file) {File Server\\Port 9010};
\node [server, below right=2cm and 1cm of frontend] (exec) {Code Exec\\Port 9012};

% Docker
\node [block, below=1.5cm of exec, fill=yellow!20] (docker) {Docker Sandbox};
...
\textbf{Concurrency Issues Resolved:}
\begin{itemize}
    \item \textbf{Problem}: Initial implementation used single buffered stream causing race conditions
    \item \textbf{Solution}: Separated read (background thread) and write (main thread) channels
    \item \textbf{Result}: No buffer conflicts, stable multi-user synchronization
\end{itemize}

\textbf{Synchronization Mechanisms:}
\begin{itemize}
    \item Python \texttt{threading.Lock()} for shared data structures
    \item Daemon threads for automatic cleanup
    \item Safe socket closure in finally blocks
\end{itemize}
...
\begin{itemize}
    \item \textbf{Lines of Code}: ~3000+ (Python)
    \item \textbf{Backend Servers}: 4
    \item \textbf{Frontend Pages}: 5
    \item \textbf{Protocols Implemented}: 4
    \item \textbf{Algorithms}: 2 (Tahoe, Reno)
    \item \textbf{Supported Languages}: 4 (Python, C, C++, Java)
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Message history and search
        \item User authentication system
    \end{itemize}
    
    \item \textbf{Enhanced Security}:
    \begin{itemize}
        \item Password-protected rooms
        \item JWT-based authentication
        \item Rate limiting per user
        \item IP whitelisting
    \end{itemize}
    
    \item \textbf{Improved Collaboration}:
    \begin{itemize}
        \item Operational Transformation (OT) for conflict resolution
        \item Cursor position sharing
        \item Syntax highlighting sync
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Video/audio chat integration
        \item Screen sharing capability
        \item Whiteboard collaboration
        \item Git integration for version control
    \end{itemize}
    
    \item \textbf{Extended Code Execution}:
    \begin{itemize}
        \item Support for more languages (Go, Rust, JavaScript)
        \item Package manager integration (pip, npm)
        \item GPU support for ML workloads
        \item Notebook-style execution (Jupyter-like)
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Native mobile apps (React Native)
...
\begin{itemize}
    \item Implementation of BBR (Bottleneck Bandwidth and RTT) congestion control
    \item QUIC protocol integration for improved performance
    \item WebRTC for peer-to-peer file transfers
    \item Machine learning for predictive congestion control
    \item Blockchain for decentralized room management
\end{itemize}

% ======================== CONCLUSION ========================
\section{Conclusion}

SyncroX represents a comprehensive exploration of networking principles through practical implementation. The project successfully bridges the gap between theoretical computer networking concepts and real-world application development.

\subsection{Learning Outcomes}

Through this project, we gained in-depth understanding of:
\begin{itemize}
    \item \textbf{Protocol Design}: Creating efficient, robust application-layer protocols
    \item \textbf{Congestion Control}: Implementing and comparing Tahoe and Reno algorithms
    \item \textbf{Concurrent Programming}: Building thread-safe multi-client servers
    \item \textbf{System Security}: Containerization and resource isolation techniques
    \item \textbf{Real-time Systems}: Synchronization and state management across distributed clients
\end{itemize}

\subsection{Project Impact}

SyncroX serves multiple purposes:
\begin{enumerate}
    \item \textbf{Educational Tool}: Demonstrates networking concepts in action
    \item \textbf{Functional Platform}: Provides real collaboration capabilities
    \item \textbf{Foundation for Research}: Extensible architecture for future enhancements
    \item \textbf{Portfolio Piece}: Showcases full-stack development skills
\end{enumerate}

\subsection{Technical Success}

The project successfully:
\begin{itemize}
    \item Implements four independent TCP servers with custom protocols
    \item Demonstrates measurable differences between congestion control algorithms
    \item Provides secure, isolated code execution environment
    \item Handles concurrent users with thread-safe operations
    \item Visualizes complex networking metrics in real-time
\end{itemize}

\subsection{Acknowledgment of Challenges}

Building SyncroX presented several challenges:
\begin{itemize}
    \item \textbf{Concurrency}: Resolved race conditions through careful thread design
    \item \textbf{Binary Protocols}: Handled mixed text/binary data correctly
    \item \textbf{Docker Integration}: Managed stdin/stdout with containerized processes
    \item \textbf{Real-time Sync}: Balanced update frequency with performance
\end{itemize}

Each challenge provided valuable learning experiences and deeper understanding of networking fundamentals.

\subsection{Final Thoughts}

SyncroX demonstrates that complex networking systems can be built using foundational principles and modern tools. The project proves that theoretical concepts like congestion control, RTT estimation, and protocol design are not just academic exercises—they are essential building blocks for real-world distributed systems.

We hope SyncroX serves as a valuable resource for students learning computer networking and inspires further exploration of these fascinating topics.

\vspace{1cm}

\begin{center}
\textbf{Thank you for reviewing our project!}
\end{center}
...
\begin{enumerate}
    \item Computer Networking: A Top-Down Approach, Kurose \& Ross, 8th Edition
    \item TCP/IP Illustrated, Volume 1: The Protocols, W. Richard Stevens
    \item Python Official Documentation: Socket Programming \\
    \url{https://docs.python.org/3/library/socket.html}
    \item Docker Documentation: Container Security \\
